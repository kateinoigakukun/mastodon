...
module PG
  class ConnectionBad < StandardError; end
end
...
class Integer
  unless method_defined?(:rails_web_original_to_js)
    alias_method :rails_web_original_to_js, :to_js
  end
  def to_js
    if self.bit_length > 32
      return self.to_s
    end
    rails_web_original_to_js
  end
end
